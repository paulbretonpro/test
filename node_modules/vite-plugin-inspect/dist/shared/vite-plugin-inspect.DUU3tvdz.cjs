...
function requireGracefulFs () {
	...
	var fs = require$$0__default$2;
	var polyfills = requirePolyfills();
	var legacy = requireLegacyStreams();
	var clone = requireClone();

	var util = require$$4__default;
	...
	if (!fs[gracefulQueue]) {
	  ...
	  if (/\bgfs4\b/i.test(process.env.NODE_DEBUG || '')) {
	    process.on('exit', function() {
	      ...
	      require$$5__default.equal(fs[gracefulQueue].length, 0);
	    });
	  }
	}
...
async function generateBuild(ctx) {
  ...
  await fs.copy(dirs.DIR_CLIENT, targetDir);
  await Promise.all([
    fs.writeFile(
    ...
    ...[...ctx._idToInstances.values()].flatMap(
      ...
      await fs.ensureDir(node_path.join(reportsDir, key));
      await fs.ensureDir(node_path.join(reportsDir, key, "transforms"));
...
const normalizePathRegExp = new RegExp(`\\${require$$1.win32.sep}`, 'g');
const normalizePath = function normalizePath(filename) {
    return filename.replace(normalizePathRegExp, require$$1.posix.sep);
};

function getMatcherString(id, resolutionBase) {
    if (resolutionBase === false || require$$1.isAbsolute(id) || id.startsWith('**')) {
        return normalizePath(id);
    }
    // resolve('') is valid and will default to process.cwd()
    const basePath = normalizePath(require$$1.resolve(resolutionBase || ''))
    ...
    return require$$1.posix.join(basePath, normalizePath(id));
}
