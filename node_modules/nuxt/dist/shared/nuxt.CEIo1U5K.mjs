...
async function augmentPages(routes, vfs, ctx = {}) {
  ...
  for (const route of routes) {
    if (route.file && !ctx.pagesToSkip?.has(route.file)) {
      const fileContent = route.file in vfs ? vfs[route.file] : fs.readFileSync(ctx.fullyResolvedPaths?.has(route.file) ? route.file : await resolvePath$1(route.file), "utf-8");
...
function serializeRouteValue(value, skipSerialisation = false) {
  if (skipSerialisation || value === undefined) {
    return undefined;
  }
  ...
  return {
    ...
    transform(code) {
      ...
      if (s.hasChanged()) {
        return {
          code: s.toString(),
          map: nuxt.options.sourcemap.client || nuxt.options.sourcemap.server ? s.generateMap({ hires: true }) : undefined
        };
      }
    }
  };
...
function findComponent(components, name, mode) {
  ...
  const component = components.find((component2) => id === component2.pascalName && ["all", mode, undefined].includes(component2.mode));
  if (component) {
    return component;
  }
...
function TransformPlugin$1(nuxt, options) {
  ...
  return createUnplugin(() => ({
    ...
    async transform(code, id) {
      ...
      return {
        code: result.code,
        map: nuxt.options.sourcemap.server || nuxt.options.sourcemap.client ? result.s.generateMap({ hires: true }) : undefined
      };
    }
  }));
}
    ...
    transform(code) {
      ...
      if (result) {
        return {
          ...
          map: options.sourcemap ? result.magicString.generateMap({ hires: true }) : undefined
        };
      }
    }
  };
...
async function initNitro(nuxt) {
  ...
  if (!nuxt.options.dev && nuxt.options.experimental.noVueServer) {
    nitro.hooks.hook("rollup:before", (nitro2) => {
      ...
      nitro2.options.renderer = undefined;
...
async function build(nuxt) {
  ...
  const generateApp$1 = debounce(() => generateApp(nuxt, app), undefined, { leading: true });
  await generateApp$1();
  if (nuxt.options.dev) {
    watch(nuxt);
    nuxt.hook("builder:watch", async (event, relativePath) => {
